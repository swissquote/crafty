...
function convertHtoRGB(hex) {
  // #<hex-color>{3,4,6,8}
  const [r, g, b, a, rr, gg, bb, aa] = (hex.match(hexColorMatch) || []).slice(
    1
  );

  if (rr !== undefined || r !== undefined) {
    const red = parseSingleOrDouble(r, rr);
    const green = parseSingleOrDouble(g, gg);
    const blue = parseSingleOrDouble(b, bb);
    const alpha = parseSingleOrDouble(a, aa, 255);
