...
test("Compiles CSS within rspack", async () => {
  const cwd = await testUtils.getCleanFixtures(
    "crafty-preset-lightningcss-rspack/compiles"
  );
...
test("Fails gracefully on broken markup", async () => {
  const cwd = await testUtils.getCleanFixtures(
    "crafty-preset-lightningcss-rspack/fails"
  );
...
test("Compiles CSS within rspack, extracts CSS ('extractCSS' boolean option)", async () => {
  const cwd = await testUtils.getCleanFixtures(
    "crafty-preset-lightningcss-rspack/extract-boolean"
  );
...
test("Compiles CSS within rspack, extracts CSS ('extractCSS' string option)", async () => {
  const cwd = await testUtils.getCleanFixtures(
    "crafty-preset-lightningcss-rspack/extract-string"
  );
...
test("Compiles CSS within rspack, extracts CSS ('extractCSS' object option)", async () => {
  const cwd = await testUtils.getCleanFixtures(
    "crafty-preset-lightningcss-rspack/extract-object"
  );
