  ...
  async loadPreset(initialConfig, preset) {
    ...
    if (!this.needsLoading(presetName)) {
      if (!this.loaded.has(resolvedModule)) {
        console.error(
          `Loading '${presetName}' failed, as the preset resolved to '${resolvedModule}'. But was apparently also loaded from another path:`,
          Array.from(this.loaded).filter(path.isAbsolute)
        );
        throw new Error(
          "Duplicate preset in your node_modules, check informations above."
        );
      }
